...
app.get('/list', (req, response) => {
  let query = 'Select location FROM battles WHERE location is NOT NULL AND location <> ""';
  con.connect(function (err) {
    if (err) {
      console.log("Error while connecting database :- " + err);
    }
...
app.get('/count', (req, response) => {
  ...
  let query = 'select count(battle_number) as count from battles;'
  con.connect(function (err) {
    if (err) {
      console.log("Error while connecting database :- " + err);
    }
...
app.get('/stats', (req, response) => {
      ...
      // query to the database;;
          let resObj = [];
          let query1 = "SELECT attacker_outcome, count(attacker_outcome) as count FROM battles GROUP BY attacker_outcome"
          ...
          con.query(query1, function(error, result1){
            ...
            let query2 = "select attacker_king, count(attacker_king) as count FROM battles GROUP BY attacker_king ORDER BY count DESC LIMIT 1;"
              ...
              let query3 = "select defender_king, count(defender_king) as count FROM battles GROUP BY defender_king ORDER BY count DESC LIMIT 1;"
              ...
              let query4 = "select region, count(region) as count FROM battles GROUP BY region ORDER BY count DESC LIMIT 1;"
              ...
              let query5 = "select name, count(name) as count FROM battles GROUP BY name ORDER BY count DESC LIMIT 1;"
              ...
              let query6 = "select distinct battle_type type FROM battles ;"
...
app.get('/battle-results', (req, res) => {
  ...
  let query = "select name from battles where" + searchquery;
